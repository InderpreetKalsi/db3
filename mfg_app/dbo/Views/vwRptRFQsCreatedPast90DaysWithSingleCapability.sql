...
--SELECT * FROM mp_rfq_parts WHERE rfq_id = 1164614
 --SELECT * FROM vwRptRFQsCreatedPast90Days  WHERE [DaysRFQCreated] < =90 ORDER BY [Buyer]
 --SELECT * FROM [vwRptRFQsCreatedPast90DaysWithSingleCapability]  WHERE [DaysRFQCreated] < =90 
 --ORDER BY [Buyer] ,RFQId
CREATE VIEW  [dbo].[vwRptRFQsCreatedPast90DaysWithSingleCapability]
AS
...
JOIN mp_contacts	b (NOLOCK) ON a.contact_id = b.contact_id AND a.rfq_status_id IN (3,5,6)  and b.IsTestAccount= 0
