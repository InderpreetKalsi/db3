...
select * from mp_contacts where user_id in
(
select id from aspnetusers where email = 'mfgsupplying@gmail.com'
)
...
*/
CREATE PROCEDURE [dbo].[proc_get_supplier_mycompanyrfqs_list]
(
	@RfqType			int,	 
	@SupplierID			int,
	@SupplierCompID		int,
		...
		begin	
			...
			FROM 
				...
				join mp_rfq b						(nolock) on mrqsq.rfq_id = b.rfq_id 
					...
					and  rfq_status_id = 6 ---- Added with M2-4670 
					and mrqsq.contact_id   in
					/* M2-2852 M - Add My Company RFQs to the left menu and new page - DB */
					(
						select supplierid from #tmp_mycompany_suppliers
