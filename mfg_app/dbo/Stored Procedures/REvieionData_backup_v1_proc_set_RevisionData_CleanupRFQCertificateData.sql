...
CREATE PROCEDURE DBO.REvieionData_backup_v1_proc_set_RevisionData_CleanupRFQCertificateData 
AS
-- =============================================
-- Author:		dp-sb
-- Create date:  18/12/2018
-- Description:	Stored procedure to set RFQ Special certificate historical data for further processing.
-- Modification:
...
BEGIN
	...
	SELECT 
		tablename + '' +  COALESCE(
					(select top 1 [value] FROM OPENJSON(field) a where [key]='RfqId') 
					, (select top 1 [value] FROM OPENJSON(oldvalue) a where [key]='RfqId') 
					, (select top 1 [value] FROM OPENJSON(newvalue) a where [key]='RfqId') 
	...
	WHERE  tablename in ('mp_rfq_special_certificates')	and 
				(oldvalue like '%,"RfqId":%'
				OR newvalue like '%,"RfqId":%'
				OR field like '%"RfqId":%')
		and is_processed = 0
 
--select * from #tmpJsonDataRFQ		
	...
	SELECT t.SearchVal, t.RFQ_ID, max(data_history_id) as data_history_id
      ,'{' + STUFF(( SELECT ', ' + replace(replace(oldvalue,'}',''), '{','')
                FROM #tmpJsonDataRFQ 
                WHERE SearchVal  = t.SearchVal
                FOR XML PATH(''),TYPE)
                .value('.','NVARCHAR(MAX)'),1,2,'') + '}' AS OldValueJson
		,  '{' + STUFF(( SELECT ', ' + replace(replace(newvalue,'}',''), '{','')
                FROM #tmpJsonDataRFQ 
                WHERE SearchVal  = t.SearchVal
                FOR XML PATH(''),TYPE)
	...
	--select * from mp_data_history_working WHERE is_processed= 0 and  tablename in ('mp_rfq_special_certificates', 'mp_rfq')
