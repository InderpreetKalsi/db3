...
/*
	...
	SELECT response_id, created_date ,score , REPLACE(comment,'"','') comment FROM mp_rating_responses (NOLOCK) WHERE to_company_id = 1686564 ORDER BY created_date DESC 

TRUNCATE TABLE XML_SupplierProfile
EXEC proc_set_XML_SupplierProfile_OneTime @PageNumber = 4 , @PageSize =5000
SELECT * FROM XML_SupplierProfile 
...
select CONVERT(INT,ROUND(ROUND(4.75, 1,0),0))
--SELECT CONVERT(INT,ROUND(ROUND(no_of_stars, 1,0),0)) ,no_of_stars FROM mp_star_rating (NOLOCK) WHERE company_id = 337027
...
CREATE PROCEDURE [dbo].[proc_set_XML_SupplierProfile_OneTime]
(
@PageNumber					INT		= 1
,@PageSize					INT		= 5000
)
...
BEGIN
	...
	DECLARE @UpToDate					DATETIME2  = '2021-01-24 23:59:59.900'
	...
	DROP TABLE IF EXISTS #XMLSupplierProfileListofCompanyIds
	DROP TABLE IF EXISTS #XML5000
	...
	WHERE a.is_buyer = 0 
		...
		AND CONVERT(DATE,b.created_date) <= '2021-01-24' 
		 --AND b.company_id = 1529366
		AND NOT EXISTS 
		(
			SELECT a1.company_id
			FROM
			(
				SELECT company_id FROM  mp_companies		(NOLOCK) WHERE [NAME] NOT LIKE '%[a-z0-9]%'
				UNION
				SELECT c.company_id 
				FROM mp_addresses (NOLOCK) a 
				JOIN mp_mst_country (NOLOCK) b ON a.country_id = b.country_id --and b.territory_classification_id = 3
				JOIN mp_contacts (NOLOCK) c on a.address_id = c.address_id
				WHERE 
				a.country_id = 247
			) a1 
			WHERE a1.company_id = b.company_id 
		)
	...
	SET @MaxDate = (SELECT MIN(created_date) FROM mp_companies (NOLOCK))
	...
	--INTO #XML5000
	FROM 
	#XMLSupplierProfileListofCompanyIds b1
