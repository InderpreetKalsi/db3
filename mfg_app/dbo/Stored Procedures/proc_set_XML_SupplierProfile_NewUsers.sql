...
CREATE PROCEDURE [dbo].[proc_set_XML_SupplierProfile_NewUsers]
(
	@MaxDate		DATETIME2,
...
BEGIN
	...
	BEGIN TRY
		...
		SET @TotalRecords = (SELECT COUNT(DISTINCT company_id ) FROM XML_SupplierProfile  WHERE isprocessed = 0 AND [action] = 'New')
		...
		VALUES ('New','proc_set_XML_SupplierProfile_NewUser','Success',NULL,NULL, @CurrentDateTime,@TotalRecords,@MaxDate, @CurrentDateTime)
	...
	BEGIN CATCH
		...
		INSERT INTO XML_SupplierProfile_Logs 
		([Action],[DBObject],[Status],[Error],[ErrorDateTime],[SuccessDateTime],[RecordProcessed],FetchDataFromDateTime,FetchDataToDateTime)
		VALUES ('New','proc_set_XML_SupplierProfile_NewUser','Fail',ERROR_MESSAGE(),@CurrentDateTime,NULL,@TotalRecords,@MaxDate, @CurrentDateTime)
