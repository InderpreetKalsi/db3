...
select top 100 * from mp_messages  order by 1 desc
select * from mp_contacts where contact_id = 1350497
select * from mp_mst_email_template where message_type_id = 240

select * from mp_rfq where rfq_id = 1156049


exec proc_set_emailMessages 
@rfq_id=1188202
,@message_type=N'rfqResponse'
,@message_status_id=0
,@from_contact=1350497
,@to_contacts=default
,@message_link=default
,@message=default

exec proc_set_emailMessages 
@rfq_id=0
,@message_type=N'SUPPLIER_PROFILE_APPROVED'
,@message_status_id=0
,@from_contact=default
,@to_contacts=1413958
,@message_link=default
,@message=N''


select * from mp_mst_message_types
select top 2 * from mp_messages nolock where rfq_id  = 1147056 order by 1 desc 
select top 2 * from mp_email_messages nolock where rfq_id  = 1147056 order by 1 desc 
select * from mp_rfq   where rfq_status_id = 2 and  rfq_id  in (select rfq_id from mp_rfq_supplier  (nolock) )


*/
CREATE PROCEDURE [dbo].[proc_set_emailMessages]
(
	@rfq_id				int = null ,
...
begin 
	...
	/*
		...
		M2-502 Buyer RFQ is released by Engineering (RFQ_RELEASED_BY_ENGINEERING 146)
		M2-598 Creating procedure to create email message and insert into email messages table (RFQ_RELEASED_BY_ENGINEERING 146)
		M2-518	Manufacturer Likes an RFQ (RFQ_LIKED_BY_SUPPLIER 148 , RFQ_DISLIKED_BY_SUPPLIER 149 )
		M2-519  Manufacturer Quotes an RFQ (rfqResponse 1 ) 


		modified on :	dec 04,2018
		M2-666 Create a stored procedure for handling & storing the notification (Email / Messages) in the relevant table
		M2-507 Buyer awards an RFQ to a manufacturer  (rfqConfirmationMessage 7 )
		M2-508 Buyer declines a quote from a manufacturer  (rfqNonConfirmationMessage 8 )
		
		modified on :	dec 05,2018
		M2-511 Buyer views a manufacturer's profile  (BUYER_VIEW_SUPPLIER_PROFILE 150)
		M2-522 Manufacturer views a buyer profile (SUPPLIER_VIEW_BUYER_PROFILE 151)
		M2-548 Buyer directly invites the manufacturer to an RFQ API (RFQ_BUYER_INVITATION 152)
		M2-506 Buyer edits an RFQ after a manufacturer marks it for quoting or quotes it (RFQ_EDITED_RESUBMIT_QUOTE 153)

		modified on :	dec 06,2018
		M2-520 Buyer has new NDA's to approve (MESSAGE_TYPE_CONFIDENTIALITY_AGREEMENT  42)

		modified on :	dec 22,2018
		M2-516 Manufacturer sends a message to the buyer (rfqFreeMessage)
		M2-503 Buyer sends a message to the manufacturer (rfqFreeMessage)
		M2-515 Buyer directly invites the manufacturer to an RFQ (RFQ_BUYER_INVITATION)
		M2-521 Manufacturer receives a notification that there are new RFQ's match their capabilities (RFQ_RELEASED_BY_ENGINEERING)
		M2-775 Vision - Add an Incomplete RFQ button in Vision on the RFQ Details page- API  (RFQ_MARKED_INCOMPLETE)

		modified on :	dec 22,2018
		M2-504 Buyer approves a 2nd level NDA
		M2-505 Buyer declines a 2nd level NDA
		M2-508 Buyer declines a quote from a manufacturer
		M2-517 Manufacturer marks an RFQ for Quoting
		M2-519 Manufacturer Quotes an RFQ
	*/
	set nocount on
